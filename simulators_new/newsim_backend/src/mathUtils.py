...
def deg2meters(sim, object_origin, object_dest):
    ...
    # may want to use deltaDeg() for this
    #   first bit since the code is copypasted
    ...
    lon_deg_length = 111412.84 * \
        math.cos(2*olat_deg)-93.5*math.cos(3*olat_deg) + \
        0.118*math.cos(5*olat_deg)
    # used wikipedia for the longitude deg size in meters
    # olat_deg was use as opposed to rlat_deg arbitrarily,
    #   since it has no significant bearing
...
def meters2deg(sim, distance, angle):
    ...
    lon_deg_length = 111412.84 * \
        math.cos(2*lat_deg)-93.5*math.cos(3*lat_deg) + \
        0.118*math.cos(5*lat_deg)
